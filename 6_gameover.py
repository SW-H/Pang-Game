...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                ...
                character_to_x += character_speed
            elif event.key == pygame.K_SPACE:  # 스페이스바 눌렀을 때 - 무기 발사
                weapon_x_pos = character_x_pos + character_width / 2 - weapon_width / 2  # 캐릭터의 중앙으로 무기 발사 위치 지정
    ...
    # 공 위치 정의
    for ball_idx, ball_val in enumerate(balls):  # index정보와 그 값을 가져옴
        ...
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ...
            ball_val["to_y"] += 0.5  # 속도(음수)를 줄여주는 효과 -> 포물선 형태
    ...
    for ball_idx, ball_val in enumerate(balls):  # index정보와 그 값을 가져옴
        ...
        break  # 안쪽 for 문에서 break를 만나면 여기로 진입 가능. 2중 for 문을 한번에 탈출하는 효과
